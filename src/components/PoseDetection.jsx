...
export default function PoseDetection({ exercise, onStatsUpdate }) {
  ...
  useEffect(() => {
    ...
    const analyzeSquat = (landmarks, ctx, canvas) => {
      ...
      // Smooth the angle
      const alpha = 0.3
      stateRef.current.smoothedAngle = alpha * kneeAngle + (1 - alpha) * stateRef.current.smoothedAngle
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
        ...
        <p className="text-[var(--color-accent)] font-medium mb-2">Camera Error</p>
